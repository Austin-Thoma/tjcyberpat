function initialize() {
    ...
    // year picker
    var yearUrls = {
        2023: "https://sheets.googleapis.com/v4/spreadsheets/10p7tyg-T9F2RiRTVxD55yFNhPl1qi2RElWgM7ZrJJlU/?key=AIzaSyCTbB_t6RGvXxjBzaV3EOUsUh3xbr0QeQc&includeGridData=true",
    ...
    $(document).ready(function () {
        displayLectures($(".select-year .selected").attr("year"))
    });
    ...
    function displayLectures(year) {
        year = parseInt(year);
        if (lectureData[year] == undefined) {
            $.ajax({
                url: yearUrls[year],
                type: "get",
                async: false,
                success: function (data) {
                    lectureData[year] = data;
                }
            });
        }

        data = lectureData[year];
